...
public class TramTest {
    ...
    public void moveTest() throws IOException, JSONException {
        ...
        for(Tramline tramline: gameState.allTramlines){
            ...
            assertTrue("Valid descending moving", tram.sectorOn == tramline.sectors.get(0) && tram.getDirection() == 0);
            ...
            for(int i = 0; i < size-1; i++){ tram.makeMove(time, gameState); }
            assertTrue("Valid ascending moving", tram.sectorOn == tramline.sectors.get(size-1) && tram.getDirection() == 1);
            tram.makeMove(time, gameState);
